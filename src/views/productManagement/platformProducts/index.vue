            ...
            <el-row :gutter="10" class="mb8">
              <el-col :span="1.5">
                <el-button type="primary" plain icon="Plus" @click="handleAdd"
                  v-hasPermi="['productManagement:platformProducts:add']">新增</el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate"
                  v-hasPermi="['productManagement:platformProducts:edit']">修改</el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
                  v-hasPermi="['productManagement:platformProducts:remove']">删除</el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="info" plain icon="Upload" @click="handleImport"
                  v-hasPermi="['productManagement:platformProducts:import']">导入</el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="warning" plain icon="Download" @click="handleExport"
                  v-hasPermi="['productManagement:platformProducts:export']">导出</el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="primary" plain icon="el-icon-upload2" :disabled="multiple" @click="importCompany"
                  v-hasPermi="['productManagement:companyProducts:importCompany']">导入到公司产品库</el-button>
              </el-col>
...
<script setup name="PlatformProducts">
import { getToken } from "@/utils/auth"
import useAppStore from '@/store/modules/app'

import { listCategory } from "@/api/system/category"
import { platformProducts_list, addPlatformProducts, updatePlatformProducts, rolePlatformProducts, delPlatformProducts, importToCompany } from "@/api/productManagement"
...
function handleDelete (row) {
  ...
  proxy.$modal.confirm('是否确认删除产品编号为"' + productIds + '"的数据项？').then(function () {
    return delPlatformProducts(productIds)
  }).then(() => {
    getList()
...
/** 导入到公司产品库 */
function importCompany (row) {
  const productIds = row.productId || ids.value
  proxy.$modal.confirm('是否确认产品编号为"' + productIds + '"导入到公司产品库？').then(function () {
    return importToCompany(productIds)
  }).then(() => {
    getList()
...
function handleUpdate (row) {
  ...
  rolePlatformProducts(productId).then(response => {
  ...
  proxy.$refs["userRef"].validate(valid => {
    if (valid) {
      if (form.value.productId != undefined) {
        updatePlatformProducts(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addPlatformProducts(form.value).then(response => {

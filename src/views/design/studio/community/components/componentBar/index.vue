<template>
  <el-dialog ref="dialogRef" class="datavis-resource-dialog" modal-class="datavis-editor-hide-modal" draggable
    v-model="state.visible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false"
    :lock-scroll="false" :modal="false" width="278" @open="setPosition">
    <template #header>
    ...
    <div class="tabs-scroll-wrapper">
      <div class="btns-wrap">
        <div class="btn" v-for="item in state.mainTabs" :key="item.name"
          :class="{ active: state.mainActive === item.name }" @click="handleTabChange(item.name)">
          {{ item.title }}
        </div>
      </div>
    </div>
    <div class="tabs-scroll-wrapper" :class="{ 'none-shadow': state.listData.length }" v-if="state.mainActive">
      <el-scrollbar max-height="85px">
        <div class="text-wrap">
          <div class="text" v-for="item in state.subTabs" :key="item.id"
            :class="{ active: state.subActive === item.name }" @click="handleSubTabChange(item)">
            {{ item.title }}
          </div>
        </div>
      </el-scrollbar>
    ...
    <div class="scrollbar-wrap">
      <el-scrollbar>
        <div class="list" ref="listRef">
          <div class="list-item" v-for="item in state.listData" :key="item.id" draggable="true"
...
const initSubData = () => {
  ...
  console.log(state.listData, ' 1122state.listData');

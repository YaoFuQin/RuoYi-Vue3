<template>
  <div class="datavis-editor-data-table">
    <el-table ref="dataTableRef" class="data-edit-table" :data="tableData" max-height="460"
      :header-cell-class-name="getHeaderCellClassName" @header-click="handleHeaderClick" @row-click="handleRowClick"
      ...
      <el-table-column v-for="(tHeader, indexT) in columns" :key="indexT" :prop="tHeader.prop" :label="tHeader.prop"
        min-width="50">
        <template #header="scope">
          <div :data-column-index="indexT">
            <el-input v-show="isActiveCell(-1, indexT)" ref="contentInputRef" v-model="columns[indexT].label"
              :disabled="inputDisable" :data-column-index="indexT"
              @blur="handleHeaderBlur(columns[indexT], columns[indexT].label)" size="small" placeholder="" />
            <span v-show="!isActiveCell(-1, indexT)">{{ columns[indexT].label }}</span>
          </div>
        </template>
        <template #default="scope">
          <el-input :disabled="inputDisable" :class="{ current: isActiveCell(scope.$index, indexT) }"
            :type="tHeader.dataType === 'number' ? 'number' : 'text'" :data-row-index="scope.$index"
            :data-column-index="indexT" v-model="scope.row[tHeader.prop]" placeholder="">
          </el-input>
        </template>
      </el-table-column>
    </el-table>

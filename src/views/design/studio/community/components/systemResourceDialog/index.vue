<template>
  <el-dialog v-bind="attrs" ref="dialogRef" class="datavis-resource-dialog" modal-class="datavis-editor-hide-modal"
    draggable v-model="state.visible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false"
    :lock-scroll="false" :modal="false" :width="state.dialogWidth">
    <template #header>
    ...
    <div class="tabs-scroll-wrapper">
      <div class="btns-wrap">
        <div class="btn" v-for="item in state.lv1Tabs" :key="item.id" :class="{ active: state.lv1Key === item.id }"
          @click="handleLv1Change(item)">
          {{ item.name }}
        </div>
      </div>
    </div>
    ...
    <div class="tabs-scroll-wrapper" :class="{ 'none-shadow': !state.lv3Key }" v-if="state.lv1Key">
      <el-scrollbar max-height="85px">
        <div class="text-wrap">
          <div class="text" v-for="item in state.lv2Tabs" :key="item.id" :class="{ active: state.lv2Key === item.id }"
            @click="handleLv2Change(item)">
            {{ item.name }}
          </div>
        </div>
      </el-scrollbar>
    ...
    <div class="tabs-scroll-wrapper none-shadow" v-if="state.lv2Key && state.lv3Tabs.length">
      <el-scrollbar max-height="85px">
        <div class="text-wrap">
          <div class="plain-text" v-for="item in state.lv3Tabs" :key="item.id"
            :class="{ active: state.lv3Key === item.id }" @click="handleLv3Change(item)">
            {{ item.name }}
          </div>
        </div>
      </el-scrollbar>
    ...
    <div class="scrollbar-wrap">
      <el-scrollbar>
        <div class="list" ref="listRef">
          <div class="list-item" v-for="item in state.dataList" :key="item.id" :draggable="canDropToCanvas"
            @dragstart="handleDragStart($event, item)" @click="handleAddCom(item)">

<template>
  <div class="datavis-contextmenu-bar" :style="{
    left: `${state.mousePosition.x}px`,
    top: `${state.mousePosition.y}px`,
    position: 'fixed'
  }" v-show="state.rightMenuShow" v-click-outside="onClickOutside" @contextmenu.prevent>
      ...
      <template v-else>
        ...
        <template v-else>
          <template v-if="state.selectFlag">
            <contentmenu-item v-if="state.lockFlag" title="解锁"
              @click="handleCommand($event, operationTypes.unlock)"></contentmenu-item>
            <contentmenu-item v-else title="锁定" @click="handleCommand($event, operationTypes.lock)"></contentmenu-item>
            <contentmenu-item v-if="state.visibleFlag" title="隐藏"
              @click="handleCommand($event, operationTypes.visible, false)"></contentmenu-item>
            <contentmenu-item v-else title="显示"
              @click="handleCommand($event, operationTypes.visible, true)"></contentmenu-item>
            ...
            <contentmenu-item title="下移"
              @click="handleCommand($event, operationTypes.sendBackwards)"></contentmenu-item>
            <contentmenu-divider />
            <contentmenu-item title="删除" @click="handleCommand($event, operationTypes.delete)"></contentmenu-item>
          </template>
        </template>
